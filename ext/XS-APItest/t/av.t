#!perl

use v5.36;

use Test::More tests => 42;
use XS::APItest;

# code stolen from t/op/hash.t
sub guard::DESTROY {
    ${ $_[0] }->();
}
sub guard :prototype(&) {
    my $callback = shift;
    return bless \$callback, "guard";
}

# to insert
{
    my @arr;

    @arr = ('a' .. 'd');
    av_splice @arr, 0, 0, '<';
    is_deeply \@arr, ['<', 'a', 'b', 'c', 'd'];

    @arr = ('a' .. 'd');
    av_splice @arr, 2, 0, '|';
    is_deeply \@arr, ['a', 'b', '|', 'c', 'd'];

    @arr = ('a' .. 'd');
    av_splice @arr, 4, 0, '>';
    is_deeply \@arr, ['a', 'b', 'c', 'd', '>'];

    # insert multiple

    @arr = ('e' .. 'h');
    av_splice @arr, 0, 0, 'a' .. 'd';
    is_deeply \@arr, ['a'..'h'];

    @arr = ('a', 'b',  'g', 'h');
    av_splice @arr, 2, 0, 'c' .. 'f';
    is_deeply \@arr, ['a'..'h'];

    @arr = ('a' .. 'd');
    av_splice @arr, 4, 0, 'e' .. 'h';
    is_deeply \@arr, ['a'..'h'];

    # insert into prealloc area
    @arr = (undef, 'b' .. 'e');
    shift @arr;
    av_splice @arr, 0, 0, 'a';
    is_deeply \@arr, ['a' .. 'e'];

    # insert into mix of prealloc and Move()d area
    shift @arr;
    av_splice @arr, 0, 0, 'Y', 'Z', 'a';
    is_deeply \@arr, ['Y', 'Z', 'a' .. 'e'];

    # we can't really unit-test if this used the prealloc area but at least we
    # hope not to crash or upset valgrind
}

# insert idx from end
{
    my @arr;

    @arr = ('a' .. 'd');
    av_splice @arr, -2, 0, '|';
    is_deeply \@arr, ['a', 'b', '|', 'c', 'd'];

    @arr = ('a' .. 'd');
    av_splice @arr, -4, 0, '<';
    is_deeply \@arr, ['<', 'a', 'b', 'c', 'd'];
}

# to replace
{
    my @arr;

    @arr = ('a' .. 'e');
    av_splice @arr, 0, 1, 'A';
    is_deeply \@arr, ['A', 'b', 'c', 'd', 'e'];

    @arr = ('a' .. 'e');
    av_splice @arr, 2, 1, 'C';
    is_deeply \@arr, ['a', 'b', 'C', 'd', 'e'];

    @arr = ('a' .. 'e');
    av_splice @arr, 4, 1, 'E';
    is_deeply \@arr, ['a', 'b', 'c', 'd', 'E'];
}

# to delete
{
    my @arr;

    @arr = ('a' .. 'e');
    av_splice @arr, 0, 1;
    is_deeply \@arr, [     'b', 'c', 'd', 'e'];

    @arr = ('a' .. 'e');
    av_splice @arr, 2, 1;
    is_deeply \@arr, ['a', 'b',      'd', 'e'];

    @arr = ('a' .. 'e');
    av_splice @arr, 4, 1;
    is_deeply \@arr, ['a', 'b', 'c', 'd'     ];

    # delete multiple

    @arr = ('a' .. 'e');
    av_splice @arr, 0, 3;
    is_deeply \@arr, [               'd', 'e'];

    @arr = ('a' .. 'e');
    av_splice @arr, 1, 3;
    is_deeply \@arr, ['a',                'e'];

    @arr = ('a' .. 'e');
    av_splice @arr, 2, 3;
    is_deeply \@arr, ['a', 'b',              ];
}

# delete invokes destructor immediately
{
    my $destroyed;

    my @arr = ( guard { $destroyed++ } );

    ok !$destroyed, 'Not destroyed before av_splice as delete';
    av_splice @arr, 0, 1;
    ok $destroyed, 'Destroyed after av_splice as delete';

    undef $destroyed;
    (sub {
        ok !$destroyed, 'Not destroyed before av_splice as delete on @_';
        av_splice @_, 0, 1;
    })->( guard { $destroyed++ } );
    ok $destroyed, 'Destroyed after av_splice as delete on @_';
}

# to extract
{
    my @arr;

    @arr = ('a' .. 'e');
    is_deeply [av_splice @arr, 0, 1], ['a'];

    @arr = ('a' .. 'e');
    is_deeply [av_splice @arr, 2, 1], ['c'];

    @arr = ('a' .. 'e');
    is_deeply [av_splice @arr, 4, 1], ['e'];
}

# extract invokes destructor later
{
    my $destroyed;

    my @arr = ( guard { $destroyed++ } );

    ok !$destroyed, 'Not destroyed before av_splice as extract';
    my @ret = av_splice @arr, 0, 1;
    ok !$destroyed, 'Not destroyed after av_splice as extract';
    @ret = ();
    ok $destroyed, 'Destroyed after result array cleared';
}

# delete count bounding
{
    my @arr = ('a' .. 'e');
    is_deeply [av_splice @arr, 2, 15], ['c', 'd', 'e'];
    is_deeply \@arr, ['a', 'b'];

    # nothing left
    is_deeply [av_splice @arr, 2, 1], [];
    is_deeply \@arr, ['a', 'b'];
}

{
    my @arr = 1 .. 3;

    ok !eval { av_splice @arr, 5, 1; } and
        like $@, qr/^Modification of non-creatable array value attempted, subscript 5 /;

    ok !eval { av_splice @arr, -12, 1; } and
        like $@, qr/^Modification of non-creatable array value attempted, subscript -9 /;
}

# on tied arrays
{
    my $wanted;
    my @args_in;
    my @values_out;

    package TiedArray {
        sub TIEARRAY { return bless [] }
        sub SPLICE {
            (undef, @args_in) = @_;
            $wanted = wantarray;
            return @values_out;
        }
    }

    tie my @tiedarr, "TiedArray";

    @values_out = ("values", "out");
    is_deeply [av_splice @tiedarr, 2, 2, "args", "in"], ["values", "out"],
        'values returned from tied SPLICE method';
    is_deeply [@args_in], [2, 2, "args", "in"],
        'args in to tied SPLICE method';
    ok $wanted, 'tied SPLICE method invoked in list context';

    av_splice @tiedarr, 1, 0, "void", "context";
    is_deeply [@args_in], [1, 0, "void", "context"],
        'args in to tied SPLICE method in void context';
    ok !defined $wanted, 'tied SPLICE method invoked in void context';
}

{
    use Tie::Array;

    package TwiceSplice {
        use parent -norequire => qw(Tie::StdArray);

        sub SPLICE ($this, @all) {
            return ( map {; 1, 2, } $this->SUPER::SPLICE(@all) ); # the evil bit
        }
    }

    tie my @x, "TwiceSplice";
    @x = "a" .. "z";

    is_deeply [av_splice(@x, 0, 5)], ['a', 'a', 'b', 'b', 'c', 'c', 'd', 'd', 'e', 'e'],
        'result of TwiceSplice extract';
}
