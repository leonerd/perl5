...
L<C<"trim"> feature|feature/The 'trim' feature> is available only if the
trim feature is enabled or if it is prefixed with C<CORE::>.  The trim
function is enabled automatically with a C<use v7> (or higher) declaration
in the current scope.

Trim is only available on Perl v7.0 and newer. For older Perl versions
look at the String::Util module for a comparable implementation.
